...
- The visible `_F` marker (`_F <ConceptName>:`) declares which concept the element belongs to. For invisible markers, use the HTML comment alternative: `<!-- _F <ConceptName>: -->` followed by the element name.
...
FORMAT supports three matrix types, distinguished by the section name in `_F matrices:`:
