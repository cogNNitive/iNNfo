...
- [ ] 2.1 Enforce Element uniqueness within the whole Model; Concept within Model; Model within Workspace (R-IE-02).
- [ ] 2.2 Remove the silent `#2` occurrence suffix in `identity.ts`; collisions become errors (R-IE-02).
- [ ] 2.3 Introduce the single mutation entry point (`applyMutation`) in `innfo-core` (R-IE-01).
- [ ] 2.4 Add reserved-name validation for `Concepts`/`Elements`/`Markers` (R-MM-02).

## Phase 3 — Rename + validator

- [ ] 3.1 Implement transactional `renameEntity` rewriting marker, index, matrix labels, `reference` fields, graph edges, cross-model refs (R-IE-03).
- [ ] 3.2 Implement `validateReferences` (dangling-reference detection) with human-friendly diagnostics (R-IE-04).
- [ ] 3.3 Apply the unified diagnostic policy (ERROR structural / WARNING recoverable / nothing silent) (R-IE-05).
- [ ] 3.4 Implement derived slug convention (NFKD transliteration; never persisted) (R-IE-06).

## Phase 4 — Clients + cleanup

- [ ] 4.1 Refactor MCP `applyChange` to delegate to the core engine (R-IE-01).
- [ ] 4.2 Refactor the editor UI mutation paths to call the core engine (R-IE-01).
- [ ] 4.3 Remove FOLDER KB starter (`HomeView.vue`), MCP `_F.md` probing, and `helpers.ts` FOLDER type.
- [ ] 4.4 Retire hierarchy-matrix handling in `parser/graph.ts` (R-MM-07).
- [ ] 4.5 Update template/model matrices to `values` (reader tolerance for legacy `params` if needed).

## Phase 5 — Tests & verification

- [ ] 5.1 Unit: uniqueness-as-error, no `#2`, slug transliteration.
- [ ] 5.2 Unit: transactional rename propagation across all reference kinds.
- [ ] 5.3 Unit: reference-integrity validator on hand-edited fixtures.
- [ ] 5.4 Contract: UI and MCP reject the same duplicate identically.
- [ ] 5.5 Regression: full parser/validator suite across the monorepo.
- [ ] 5.6 Verify each R-MM / R-IE requirement against the rewritten artifacts.
