...
describe('Multi-Store Template Resolver (innfo-core)', () => {
  ...
  it('resolves workspace-local template first (Precedence 1)', async () => {
    ...
    fs.writeFileSync(path.join(skillsDir, 'nn-innfo', 'templates', `${tmplName}.md`), '# NN concept: Skill')

    const loc = await resolveTemplatePath(tmplName, { workspaceDir, globalTemplatesDir: globalDir, skillsDir })
    expect(loc).not.toBeNull()
    expect(loc?.source).toBe('workspace')
    expect(loc?.filePath).toBe(path.join(workspaceDir, 'templates', `${tmplName}.md`))

    const locSync = resolveTemplatePathSync(tmplName, { workspaceDir, globalTemplatesDir: globalDir, skillsDir })
    expect(locSync?.source).toBe('workspace')
  })
  ...
  it('throws UnresolvedTemplateError with searched paths diagnostics when template is missing everywhere', () => {
    const missingName = 'non_existent_spec'
    const checkedPaths = [
      path.join(workspaceDir, 'templates', `${missingName}.md`),
      path.join(globalDir, `${missingName}.md`),
...
`
    ...
    const templateDoc: SpecDocument = {
      ...
      frontmatter: { level: 2, spec_version: 'V_1-0-0', spec_url: 'https://example.com/projects_V_0-1-0_NN.md' },
    }
