...
describe('business-model_V_0-1-0 — composite of organization + projects', () => {
  ...
  it('keeps its own concepts and drops the ones that left', () => {
    ...
    for (const gone of ['Analysis', 'Validation', 'Persons', 'Positions', 'Milestones', 'Project plan']) {
  ...
  it('no longer declares the moved concepts in its own body', () => {
    ...
    for (const moved of ['Roles', 'Functions', 'Skills', 'Persons', 'Positions', 'Milestones', 'Phases', 'Analysis', 'Validation']) {
  ...
  it('pulls the human-structure and project concepts back via includes', () => {
    ...
    for (const viaInclude of [
      'Organization', 'Roles', 'Functions', 'Position', 'Person', 'Skills',
      'Project', 'Phases', 'Project roles',
    ]) {
...
describe('analysis_V_0-1-0 — sample model validates', () => {
  it('the StartupValidation sample has no "not defined in template" errors', () => {
    const modelContent = readSpec('templates/analysis/samples/StartupValidation_V_0-1-0_analysis_NN.md')
...
describe('business_V_0-2-0 — umbrella composite (D1 marker dedup)', () => {
  ...
  it('unions concepts from all four templates', () => {
    ...
    const names = new Set(schema.concepts.map((c) => c.name))
    for (const n of [
      'Business summary', 'Team', 'Procedure', // business-model
      'Analysis', 'Validation', 'SWOT', // analysis
      'Organization', 'Person', 'Skills', // organization
      'Project', 'Phases', 'Project roles', // projects
    ]) {
...
describe('Ghostbusters_V_0-2-0 — sample validates against the composed umbrella', () => {
  it('parsing the sample against the resolved umbrella yields no "not defined in template" errors', () => {
    ...
    const undefinedConcept = result.errors.filter((e) => /is not defined in template/.test(e.message))
