...
describe('listModels (recursive scan)', () => {
  ...
  it('finds nested model files recursively and skips ignored directories', async () => {
    ...
    await mkdir(join(root, '.spec-cache'), { recursive: true })
    await writeFile(join(root, '.spec-cache', 'Cached_V_0-0-1_NN.md'), '# Cache', 'utf-8')
    ...
    expect(ids).not.toContain('Cached_V_0-0-1_NN')
