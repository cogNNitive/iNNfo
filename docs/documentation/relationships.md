...
En iNNfo, las relaciones entre elementos y conceptos se clasifican en **3 niveles formales**, dependiendo de su origen y grado de estructuración.

---

## Los 3 Niveles de Relaciones
...
```
         ...
         ▼                             ▼                             ▼
┌───────────────────┐         ┌───────────────────┐         ┌───────────────────┐
│ 1. ESTRUCTURAL    │         │ 2. ATRIBUTO       │         │ 3. CONTEXTUAL     │
│    (Matriz)       │         │    (Campo)        │         │    (Mención)      │
└────────┬──────────┘         └────────┬──────────┘         └────────┬──────────┘
         │                             │                             │
  Matriz de Dominio             Campo referencial             Wikilink libre
  (ej. WORK → ROLES)            (key:: [[target]])            ([[Target Element]])
```

### 1. Relaciones Estructurales (Matrices)
* **Origen**: Celdas de intersección declaradas en matrices formales del metamodelo (ej. `WORK → ROLES` o `WORK → ARTIFACT`).
* **Sintaxis**: Declaración centralizada en cabecera de metamodelo (`matrices`) e intersecciones en bloques.
* **Icono Visual**: 📊 `LayoutGrid`
* **Ejemplo**: Un procedimiento `Solicitud de Visado` asignado con el rol `Responsible` hacia `Inés (viajera)`.

### 2. Relaciones por Atributo (Campos Referenciales)
* **Origen**: Campos estructurados en los bloques cuyos valores hacen referencia explícita a otro elemento.
* **Sintaxis**: `campo:: [[nombre_del_elemento]]` o campos tipados como `reference`.
* **Icono Visual**: 🏷️ `Tag`
* **Ejemplo**: `depends_on:: [[Verificación de Pasaporte]]` o `assigned_to:: [[Padres]]`.

### 3. Relaciones Contextuales (Menciones & Wikilinks)
...
* **Ejemplo**: `> ( 🏷️ depends_on ) > [Verificación de Pasaporte]`
...
* **Ejemplo**: `[Solicitud de Visado] < ( 📊 Responsible ) <`
