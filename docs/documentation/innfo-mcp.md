...
`@innv0/innfo-mcp` is an MCP (Model Context Protocol) server that wraps `@innv0/innfo-core` and exposes iNNfo models over the stdio transport, so AI coding agents — Claude Code, OpenCode, and others — can read, validate, and edit iNNfo models through structured tool calls instead of freeform file edits.
...
```
AI coding agent (Claude Code, OpenCode, ...) → innfo-mcp (MCP server, stdio) → @innv0/innfo-core
                                                        ↓
...
```bash
# Build the server
npm run build -w @innv0/innfo-mcp
