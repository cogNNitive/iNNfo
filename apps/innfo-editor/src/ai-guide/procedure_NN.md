...
* _NN Work: Use cogNNitive with AI
  cogNNitive lets you edit and view iNNfo models both from its graphical interface and through AI agents. This procedure describes how to use **OpenCode** (the supported AI agent) to work with your models via natural language conversation, leveraging the contextual prompts the application provides.
...
* _NN Work: Download and install OpenCode
  ...
  tool: "OpenCode"
  ```
  Download OpenCode from https://github.com/anomalyco/opencode and install it. OpenCode is the supported AI agent for cogNNitive — it reads project skills natively and discovers them automatically.
* _NN Work: Open the workspace folder in OpenCode
  ...
  tool: "OpenCode"
  ```
  Start OpenCode in the same workspace folder you use in cogNNitive. You can find the exact path at the top of the header by clicking the info icon. OpenCode works directly on the file system.
* _NN Work: Configure MCP tools
  ...
  requires: MCP Server
  ```
  The first time you work with models, tell OpenCode: *"innfo: Load the innv0-innfo skill and check that innfo-mcp is configured"*. The skill detects if the MCP server is set up and guides you through any steps if needed. Reference: `docs/mcp-setup.md`.
* _NN Work: Edit models via chat
  ...
  tool: "OpenCode"
  ```
  Tell OpenCode what you want to do including a reference to the skill you need, for example: *"innfo: Load the innv0-innfo skill — I need to edit the business model and add a new concept"*. The skill reference in your message helps OpenCode discover and activate the right skill automatically. The skill provides model validation, MCP activation, and change workflows.
* _NN Work: Import documents into iNNfo models
  ...
  tool: "OpenCode"
  ```
  Place the documents you want to transform in **traNNsform/input/**. Then go to the **Import** panel in cogNNitive and copy the generated prompt — it tells OpenCode exactly what to do. The **innv0-trannsform** skill handles document ingestion, normalization, and conversion.
* _NN Work: Export models as HTML visualizers
  ...
  tool: "OpenCode"
  ```
  Open the model you want to visualize, then go to the **Export** panel in cogNNitive and copy the generated prompt. It includes the model name and instructions for OpenCode. The **innv0-innfo** skill handles visualizer generation. Results appear in **traNNsform/output/**. Tell OpenCode: *"innfo: Load the innv0-innfo skill — I need to generate an HTML visualizer following traNNsform/AGENT.md"*.
* _NN Work: Use the suggested prompts
  ...
  ```
  When viewing a model in cogNNitive, the right sidebar shows **suggested prompts** for each concept. Copy them into OpenCode to explore a specific concept or element in more detail.
...
* _NN Tools: OpenCode
  Supported AI agent for cogNNitive. Reads project skills natively and discovers them automatically. Download: https://github.com/anomalyco/opencode
